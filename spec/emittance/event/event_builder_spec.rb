  ...
  describe '.object_to_klass' do
    context 'with one argument' do
      ...
      it 'converts a multi-word snake-cased symbol to a camel-cased class' do
        value = Emittance::Event::EventBuilder.object_to_klass(:foo_bar)
        expect(value).to eq(FooBarEvent)
        expect(value < Emittance::Event).to be(true)
      end

      it 'creates a class where none existed' do
        value = Emittance::Event::EventBuilder.object_to_klass(:foo_bar_bar)
        expect(value).to eq(FooBarBarEvent)
        expect(value < Emittance::Event).to be(true)
      end

      it 'removes bangs and question marks' do
        value_1 = Emittance::Event::EventBuilder.object_to_klass(:foo!)
        value_2 = Emittance::Event::EventBuilder.object_to_klass(:bar?)
      ...
      it 'doesn\'t care about crazy characters or names' do
        ...
        value = Emittance::Event::EventBuilder.object_to_klass(weird_identifier)
        expect(value).to eq(FooBarEvent)
        expect(value < Emittance::Event).to be(true)
      end

      it 'can handle namespaced objects' do
        value = Emittance::Event::EventBuilder.object_to_klass(Foo::Baz)
