...
module SystemEvents::Action
  ...
  class << self
    def included(action_klass) # :nodoc:
      ...
      handler_klass.class_eval do
        ...
        def initialize(action_obj) # :nodoc:
          @action = action_obj
        end
    ...
    def handler_klass_name(action_klass) # :nodoc:
      "#{action_klass}Handler"
    end

    def emitting_event_name(action_klass) # :nodoc:
      SystemEvents::Emitter.emitting_method_event_name(action_klass, SystemEvents::Action::EMITTING_METHOD)
    end

    def find_or_create_klass(klass_name) # :nodoc:
